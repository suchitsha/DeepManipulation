...
def learnMotion():
    ...
    skip = 0
    iterations = 20      # trajectories to be generated

    #train
    training_set = process_data("/home/shar_sc/Documents/DirtDetection/data/motion/data2_40_10.p",app.output)#data2_8000_10.p",app.output)
...
def evaluate_data_from_file_dnn_poly(regressor, data, name=""):
    ...
    for i in xrange(num_points):
        ...
        if (app.initial_run == True):
            cost = tf.Variable(error, name="cost", dtype=tf.float32) #+ current prediction or Y_pred #tf.reduce_sum(tf.pow(Y_pred - Y, 2)) / (n_observations - 1)
            #learning_rate = 0.01
    ...
    print "total_error:", total_error/num_points
    app.sum_error = app.sum_error + total_error
